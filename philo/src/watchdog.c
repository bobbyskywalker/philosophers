...
/*   Updated: 2025/01/20 18:29:41 by agarbacz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_if_all_ate(t_philo **philo_arr)
{
	...
	if (philo_arr[0]->common_data->is_opt_arg)
	{
		...
		{
			if (philo_arr[i]->eat_counter >= philo_arr[i]->common_data->no_must_eat)
				already_ate++;
...
void	*watchdog_thread(void *arg)
{
	...
	while (1)
	{
		...
		while (i < philo_arr[0]->common_data->no_philo)
		{
			...
			if (philo_arr[0]->common_data->program_status && (time_in_ms
					- philo_arr[i]->last_meal > philo_arr[i]->common_data->time_to_die))
			{
